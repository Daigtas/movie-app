...
export const getRandomPlaceholder = () => {
  try {
    if (!placeholderImages.length) return FALLBACK_PLACEHOLDER;
    
    const randomIndex = Math.floor(Math.random() * placeholderImages.length);
    return placeholderImages[randomIndex];
  } catch (error) {
    console.error("Error getting random placeholder:", error);
...
export const getConsistentPlaceholder = (identifier) => {
  try {
    if (!identifier || !placeholderImages.length) return FALLBACK_PLACEHOLDER;
    ...
    console.error("Error getting consistent placeholder:", error);
    return FALLBACK_PLACEHOLDER;
  }
};
